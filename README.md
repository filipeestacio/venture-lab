...
| `capture-podcast` | From a podcast/YouTube episode URL (+ optional timestamp and a one-line hook), fetches a clean transcript with `podcast-transcript` and upserts one 🎧 Source row in your Notion — idempotent on the YouTube video id, transcript attached as a `.md`, plus a short descriptive summary (abstract + key points) in the row body so the inbox is scannable. Captures at `Captured` status only; never touches Research. Needs `configure-venture-lab` run first. |
